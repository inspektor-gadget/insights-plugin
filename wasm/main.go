...
import (
	...
	json "github.com/inspektor-gadget/ig-wasm/json"
)
	...
	run := func(mode int) func(this js.Value, args []js.Value) any {
		return func(this js.Value, args []js.Value) any {
			...
			go func() {
				...
				dsNames := make(map[uint32]string)
				...
				for {
					...
					case api.EventTypeGadgetPayload:
						...
						case datasource.TypeSingle:
							...
							onData.Invoke(dsNames[ev.DataSourceID], jsjson.Call("parse", string(jsonFormatters[ev.DataSourceID].Marshal(p))))
							ds.Release(p)
						case datasource.TypeArray:
							...
							onData.Invoke(dsNames[ev.DataSourceID], jsjson.Call("parse", string(jsonFormatters[ev.DataSourceID].MarshalArray(p))))
							ds.Release(p)
						}
					case api.EventTypeGadgetInfo:
						...
						for _, ds := range gi.DataSources {
							...
							jsonFormatters[ds.Id], err = json.New(nds, json.WithFlatten(true), json.WithShowAll(true))
							if err != nil {
								returnError(err.Error())
								return
							}
